...
# Setup logging
os.makedirs(SESSIONS_DIR, exist_ok=True)  # Ensure sessions directory exists
LOG_FILE = os.path.join(
    SESSIONS_DIR, f"monitoring_agent_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
)
logging.basicConfig(
    level=logging.INFO,
...
def main(context_file, log_file, session_id=None):
    ...
    logging.info(
        f"Conversation history initialized: {len(conversation_mgr.conversation_history)} previous exchanges"
    )
        ...
        try:
            ...
            if not api_key or api_key == "YOUR_GEMINI_KEY_GOES_HERE":
                ...
                if retry_count == 1:
                    ...
                    print(
                        "   2. Replace YOUR_GEMINI_KEY_GOES_HERE with your actual key"
                    )
                    print(
                ...
                if retry_count % 6 == 0:  # Remind every minute
                    print(
                        f"⏳ Still waiting for API key in .env file... (attempt {retry_count}/{max_retries})"
                    )
            ...
            proactive_ui = None
            if os.getenv("PROACTIVE_MONITORING", "true").lower() == "true":
                try:
                    proactive_monitor = ProactiveMonitor(log_file, SESSIONS_DIR)
                    proactive_monitor.start()
                    proactive_ui = ProactiveUI(SESSIONS_DIR)
                    logging.info("✓ Proactive monitoring enabled")
                except Exception as e:
                    logging.error(f"Failed to initialize proactive monitoring: {e}")
            ...
            retry_count += 1
            logging.error(
                f"Failed to initialize Gemini client (attempt {retry_count}/{max_retries}): {e}"
            )

            if "API key not valid" in str(e):
                print("\n❌ Invalid API key. Please check your key in .env file.")
                print(
                    f"   Current key starts with: {api_key[:20]}..."
            ...
            if retry_count >= max_retries:
                logging.critical(
                    "Failed to initialize Gemini client after all attempts. Exiting."
                )
            ...
            if os.path.exists(REFRESH_REQUEST_FILE):
                ...
                try:
                    ...
                    if generate_repo_blob(str(project_root), context_file):
                        logging.info(
                            f"Repo-blob refreshed successfully: {context_file}"
                        )
                ...
                try:
                    ...
                    if SMART_CONTEXT_ENABLED:
                        ...
                        context_builder = SmartContextBuilder(
                            str(project_root), max_context_size=MAX_CONTEXT_SIZE
                        )
                        ...
                        logging.info(
                            f"Smart context built with {len(included_files)} files"
                        )
                    ...
                    base_prompt = """You are a world-class senior software architect reviewing an AI Coding Buddy project.
...
Please provide a thoughtful, actionable response that considers both the project code and the ongoing session."""
                    ...
                    try:
                        ...
                        if (
                            len(project_context) > 50000
                        ):  # If over 50KB, use file upload
                            logging.info(
                                "Uploading project context to Gemini Files API..."
                            )
                            ...
                            if session_id in uploaded_file_tracker:
                                ...
                                try:
                                    ...
                                    logging.info(
                                        f"Deleted previous upload for session: {old_file_name}"
                                    )
                                except Exception as e:
                                    logging.warning(
                                        f"Could not delete old file {old_file_name}: {e}"
                                    )

                            context_path = os.path.join(
                                SESSIONS_DIR, f"temp_context_{session_id}.txt"
                            )
                            ...
                            logging.info(
                                f"Tracking uploaded file: {uploaded_context.name}"
                            )

                            # Adjust prompt to reference uploaded file
                            prompt = prompt.replace(
                                project_context,
                                "[Project context uploaded as file - see attached]",
                            )
                        ...
                        if uploaded_files:
                            logging.info(
                                f"Uploaded {len(uploaded_files)} file(s) to Gemini. They will auto-delete after 48 hours."
                            )
                    ...
                    if is_file_operation:
                        ...
                        try:
                            ...
                            file_ops = FileOperationResponse.model_validate_json(
                                response.text
                            )
                            ...
                            logging.info(
                                f"File operations completed: {json.dumps(result.model_dump(), indent=2)}"
                            )
                    ...
                    error_msg = (
                        f"Error processing request: {type(e).__name__}: {str(e)}"
                    )
    ...
    # Stop proactive monitoring if enabled
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--context_file", required=True, help="Path to project context file"
    )
    parser.add_argument(
        "--log_file", required=True, help="Path to Claude session log file"
    )
