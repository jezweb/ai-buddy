...
class FileScorer:
    ...
    def _score_by_intent(
        self, file_path: Path, intent: QueryIntent
    ) -> Tuple[float, str]:
...
class SmartContextBuilder:
    ...
    def build_context(
        ...
        for file_rel in scored_files:
            ...
            try:
                ...
                if len(content) > 5000 and file_rel.score < 50:
                    content = self._extract_relevant_portions(
                        content, keywords, tech_terms
                    )
                ...
                if content:
                    ...
                    if current_size + len(file_section) <= self.max_context_size:
                        ...
                        self.logger.info(
                            f"Included {file_rel.path} - Score: {file_rel.score:.1f}, Reasons: {file_rel.reasons[:2]}"
                        )
        ...
        # Log context stats
        self.logger.info(
            f"Built context - Intent: {intent}, Size: {current_size:,} bytes, Files: {len(included_files)}"
        )
    ...
    def _extract_relevant_portions(
        self, content: str, keywords: List[str], tech_terms: Dict[str, float]
    ) -> str:
