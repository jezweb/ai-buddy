...
class RepoBlobGenerator:
    ...
    DEFAULT_EXTENSIONS = {'.py', '.txt', '.md', '.sh', '.js', '.ts', '.jsx', '.tsx', 
                         '.html', '.css', '.json', '.yml', '.yaml', '.toml', '.ini',
                         '.java', '.cpp', '.c', '.h', '.hpp', '.go', '.rs', '.rb'}
    
    # Patterns to exclude
    EXCLUDE_PATTERNS = {'.ai-buddy/', '__pycache__/', '.git/', 'node_modules/', 
                       '.venv/', 'venv/', 'env/', '.env', '.pyc'}
        ...
        try:
            ...
            with open(output_path, 'w', encoding='utf-8') as output:
                ...
                if git_files is not None:
                    ...
                    self.logger.info(f"Using git to find files ({len(git_files)} tracked files)")
                    ...
                    self.logger.info("No git repository found. Using extension-based file search.")
